...
export function AuthGuard({ children, permission }) {
  ...
  useEffect(
    function () {
      ...
      } else if (!auth.loading && auth.session && auth.profile && permission && !hasPermission(auth.profile, permission)) {
  ...
  if (permission && auth.profile && !hasPermission(auth.profile, permission)) return null;

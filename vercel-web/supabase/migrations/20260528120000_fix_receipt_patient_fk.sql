-- Receipt save: never insert hh_receipts.patient_id as '' (violates hh_receipts_patient_id_fk).
-- Resolve from the billing row when the client omits patient_id (billings UI did not send it).

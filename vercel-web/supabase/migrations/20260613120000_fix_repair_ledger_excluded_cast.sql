-- Fix: hominal_repair_duty_ledger_window passed `service_day` (a date) into
-- hominal_duty_day_excluded(jsonb, text, text), which has no date overload, so
-- the canonical ledger-repair / orphan-cleanup tool always errored out and
-- could never run. As a result, ledger rows belonging to deleted / cancelled
-- duties (remarks `duty:%`) were never removed from hh_svc_entries /
-- hh_payout_charges, leaving phantom payout days (e.g. a deleted duplicate duty
-- that had materialized 5 days at a bogus ₹4/day).
--
-- Changes vs. the prior definition:
--   1. `dd.service_day::text` at the hominal_duty_day_excluded call site (the
--      function only has a (jsonb, text, text) overload).
--   2. The payout-charge insert guard now keys on partner_id (matching the
--      uq_hh_payout_charges_svc_day_partner unique constraint on
--      (svc_key, date, coalesce(partner_id,''))) instead of the partner name +
--      term, which could miss an existing row when the stored name differed
--      from the recomputed name and then violate the unique constraint.
--   3. Orphan deletion is now restricted to rows whose source duty is actually
--      deleted / cancelled / missing. Because the expected set is built only
--      from patients with an ACTIVE bill, the original "not in expected" delete
--      stripped legitimate payout/billing rows for ACTIVE duties on patients
--      whose bill was closed. The added duty-state guard prevents that.
-- The duty-ledger-write session flag set at function entry (Phase 4) is preserved.

create or replace function public.hominal_repair_duty_ledger_window(
  p_from date default (date_trunc('month'::text, (current_date)::timestamp with time zone))::date,
  p_to date default current_date,
  p_actor text default 'ledger-repair@hominal.system'::text
)
returns jsonb
language plpgsql
as $function$
...
begin
  ...
  -- Remove orphan duty:% rows ONLY when their source duty is genuinely gone
  -- (deleted / cancelled / no-show / missing). Rows whose duty is still active
  -- are never deleted here, even if the patient has no ACTIVE bill, so a manual
  -- repair can never strip legitimate pay for active duties.
  with deleted as (
    ...
    where coalesce(s.remarks, '') like 'duty:%'
      ...
      and not exists (
        select 1
        from public.hh_duties d
        where d.id = split_part(coalesce(s.remarks, ''), ':', 2)
          and d.deleted_at is null
          and upper(coalesce(d.status, '')) in ('SCHEDULED', 'IN_PROGRESS', 'COMPLETED')
      )
    returning id
  )
  ...
  with deleted as (
    ...
    where coalesce(p.remarks, '') like 'duty:%'
      ...
      and not exists (
        select 1
        from public.hh_duties d
        where d.id = split_part(coalesce(p.remarks, ''), ':', 2)
          and d.deleted_at is null
          and upper(coalesce(d.status, '')) in ('SCHEDULED', 'IN_PROGRESS', 'COMPLETED')
      )
    returning id
  ...
  with inserted as (
    ...
    where not exists (
      ...
      where coalesce(p.svc_key, '') = e.svc_key
        and coalesce(p.date, '') = e.service_day::text
        and coalesce(p.partner_id, '') = e.employee_id
    )
    and not exists (
      select 1

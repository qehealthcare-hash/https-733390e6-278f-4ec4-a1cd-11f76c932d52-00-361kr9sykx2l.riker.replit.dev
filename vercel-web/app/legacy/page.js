...
export default function LegacyCrmPage() {
  var [showFrame, setShowFrame] = useState(false);

  if (showFrame) {
    return (
      <main style={{ width: "100vw", height: "100vh", margin: 0, padding: 0, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "8px 16px", background: "#fef3c7", borderBottom: "1px solid #fde68a", color: "#92400e", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span><strong>Classic CRM</strong> — kept for service-entry diary, payout charges and historical bookkeeping. Most features are now in the modern React app.</span>
          <button
            type="button"
            onClick={function () { setShowFrame(false); }}
            style={{ padding: "4px 10px", border: "1px solid #92400e", background: "#fef3c7", color: "#92400e", cursor: "pointer", borderRadius: 4 }}
          >
            Back
          </button>
        </div>
  ...
  return (
    <main style={{ padding: 32, maxWidth: 980, margin: "0 auto" }}>
      ...
      <p className="mini-muted" style={{ marginBottom: 24 }}>
        All core workflows have been ported to the new React shell. Pick a module below to continue. The Classic
        CRM iframe is still available for the few legacy-only screens (service-entry diary, payout charges,
      ...
      <div style={{ marginTop: 32 }}>
        <button
          type="button"
          onClick={function () { setShowFrame(true); }}
